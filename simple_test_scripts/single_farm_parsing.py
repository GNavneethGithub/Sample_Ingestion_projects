...
def is_valid_header_line(line: str) -> bool:
    # Customize this as per your actual header pattern
                ...
                if inside_block:
                    if "End UserGroup" in stripped_line:
                        if found_valid_block:
                            break  # stop after first valid block
                        else:
                            inside_block = False
                            continue
                    ...
                    # --- Handle exception case: no parentheses around group_member ---
                    exception_match = re.match(r'^(\S+)\s+(\S+)\s+\((.*?)\)$', cleaned)
                    if exception_match:
                        group_name = exception_match.group(1)
                        group_members_raw = exception_match.group(2)
                        user_shares_raw = exception_match.group(3)

                        group_members = [group_members_raw.strip()]
